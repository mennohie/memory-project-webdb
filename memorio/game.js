...
Game.prototype.doTurn = function () {
  ...
  const turnMsg = messages.O_PLAYER_TURN
  ...
  this.timeoutID = setTimeout(() => {
    ...
    const timerRunOutMsg = messages.O_TIMER_RUN_OUT
...
Game.prototype.turnCard = function (cardId) {
  ...
  const msg = messages.O_TURNED_CARDS
  msg.data = { turnedCards: null, newCard: null }
  msg.data.turnedCards = this.cardGrid.turnedCards.map(e => { return e.getClientTurnedCard() })
  ...
  if (this.cardGrid.turnedCards.length > 1) {
    ...
    if (isMatch) {
      ...
      const msg = messages.O_CARD_MATCH
...
Game.prototype.end = function () {
  ...
  const winMsg = messages.O_GAME_WON_BY
...
Game.prototype.addScore = function (score) {
  const msg = messages.O_ADD_SCORE
